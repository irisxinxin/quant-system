...
<!-- CLAUDE_MIGRATION_BEGIN -->
## Claude 迁移记忆

以下内容从 Claude 项目 memory/config 迁移而来。处理本项目任务前，优先读取这些资料：

- `/Users/xin/.codex/claude-migration/projects/Users-xin-Documents-Claude-Projects-money-quant-system/memory/`

补充约束：
- 遵守本目录已有 AGENTS.md 中的 LongPort 模拟盘铁律。
- 新增或修改策略前读取 feedback_backtest_lessons、feedback_realistic_fill、feedback_typeA_optimization。

原始 Claude 会话未复制到 Git 项目；会话索引位于对应目录的 `sessions.index.json`。
<!-- CLAUDE_MIGRATION_END -->
